...
app.use((error, _req, res, _next) => {
  if (error instanceof multer.MulterError) {
    ...
    const message = error.code === 'LIMIT_FILE_SIZE'
      ? isVideo ? 'About video must be 8 MB or smaller.' : isManagedImage ? 'Image must be 3 MB or smaller.' : 'Banner image must be 3 MB or smaller.'
      : isVideo ? 'Upload an MP4 or WebM video.' : 'Upload a JPG, PNG, or WebP image.';
